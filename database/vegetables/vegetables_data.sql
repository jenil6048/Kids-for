-- 1. Create vegetables table and index
...
CREATE TABLE IF NOT EXISTS public.vegetables (
  ...
  constraint vegetables_category_id_fkey foreign KEY (category_id) references categories (id) on delete CASCADE
) TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS idx_vegetables_topic_key on public.vegetables using btree (topic_key) TABLESPACE pg_default;
...
-- Ensure vegetables category exists in categories table
INSERT INTO public.categories (category_key, title, color, is_premium, group_id, display_order)
VALUES (
  'vegetables',
  '{"en": "Vegetables", "gu": "શાકભાજી", "hi": "सब्जियां"}'::jsonb,
  '#8BC34A',
  false,
  'natures_world',
  16
...
-- 2. Populate vegetables table with data
