...
class SupabaseService {
  ...
  Future<List<TopicModel>> getTopicsForCategory(String categoryKey, int categoryId) async {
    developer.log('SupabaseService: Starting getTopicsForCategory(categoryKey: $categoryKey, categoryId: $categoryId)...');
    ...
    Future<List<TopicModel>?> tryQueryTable(String tableName, {required bool applyFilter}) async {
      developer.log('SupabaseService: tryQueryTable(tableName: $tableName, applyFilter: $applyFilter)...');
      try {
        ...
        final List<dynamic> response = await query.order('display_order');
